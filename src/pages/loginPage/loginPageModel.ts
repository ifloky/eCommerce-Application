...
import { responseCustomer } from '../../types/interfaces/login';
...
export const getAllTokens = async (email: string, password: string): Promise<string> => {
  ...
  const queryParams = `?grant_type=password&username=${encodeURIComponent(email)}&password=${encodeURIComponent(
    password,
  )}`;
  ...
  if (response) {
    setCookie('access_token', response.access_token, 48);
    setCookie('refresh_token', response.refresh_token, 48 * 30);
  }
