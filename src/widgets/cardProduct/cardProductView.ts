...
export function priceWithDiscount(elem: Product): HTMLElement {
  ...
  const price = elem.masterData ? Number(elem.masterData.current.masterVariant.prices[0].value.centAmount) / 100 : Number(elem.masterVariant?.prices[0].value.centAmount) / 100;
  const discountedPrice = elem.masterData ? Number(elem.masterData.current.masterVariant.prices[0].discounted?.value.centAmount) / 100 : Number(elem.masterVariant?.prices[0].discounted?.value.centAmount) / 100;
  const image = elem.masterData ? elem.masterData?.current.masterVariant.images[0].url : elem.masterVariant?.images[0].url;
  ...
  <div class="product-card__list">
    ...
    <div class="product-card__price-wrapper">
      <span class="${discountedPrice ? 'product-card__sale' : 'product-card__price'}">${price ? price + ' $' : ''
    }</span>
